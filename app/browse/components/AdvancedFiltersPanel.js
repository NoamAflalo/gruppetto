...
export default function AdvancedFiltersPanel({
  ...
  return (
    <div className="bg-card rounded-xl border border-line p-4 md:p-6 mb-6 md:mb-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        {/* Intensity - All Views */}
        <div>
          <label className="block text-sm font-semibold text-soft mb-2">Intensity</label>
              ...
              <button
                ...
                className={`px-2 md:px-3 py-2 rounded-lg text-xs md:text-sm font-semibold capitalize transition ${
                  advancedFilters.intensities.includes(intensity)
                    ? 'bg-brand text-white'
                    : 'bg-card2 text-soft hover:text-ink'
                }`}
              >
                {intensity}
              </button>
      ...
      {isUserFemale && (
        <div className="mt-4">
          <div className="flex items-center gap-3 bg-ground rounded-xl p-4 border border-line">
            <input
              type="checkbox"
              id="girlsOnlyFilter"
              checked={advancedFilters.girlsOnly}
              onChange={(e) => setAdvancedFilters({ ...advancedFilters, girlsOnly: e.target.checked })}
              className="w-5 h-5 rounded border-line text-pink-500 focus:ring-pink-500 focus:ring-offset-card"
            />
            <label htmlFor="girlsOnlyFilter" className="text-ink font-semibold cursor-pointer flex items-center gap-2">
              Girls Only Sessions
            </label>
          </div>
        </div>
